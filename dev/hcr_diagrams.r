...
convert_to_catch <- function(f, ssb){
    ...
    catch <- afscOM::baranov(f, equil_naa, dp_y$waa, dp_y$mort, array(joint_selret$sel, dim=c(1, 30, 2, 1, 1)))
...
hcr_colors <- c("black", "red", "blue", "darkgreen", "purple", "#51949d")
...
hcr_df_long %>%
    ...
    ggplot()+
        ...
        custom_theme+
        theme(panel.spacing.x = unit(1, "cm"))
ggsave(file.path(figures_dir, "hcr_diagrams1.jpeg"), width=10, height=6, units="in")
