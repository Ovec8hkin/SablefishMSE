...
Once an operating model (OM) object, a recruitment object, and a management procedure (MP) object have all been appropriately defined, the MSE simulation loop can almost be initiated. 
...
The `diagnostics` parameter controls whether to return observation data and estimation method output objects in addition to the standard set of MSE output arrays. By default, `diagnostics=FALSE` to save memory.

The `save` parameter controls whether to save the output of MSE simulation to disk. By default, `save=FALSE` to avoid saving large MSE output objects, but often needs to be turned on when running many combinations of OMs, MPs, and simulation seeds. The default location of saved outputs is "./data/active/".

### Example

For example, the following code will run 10 simulations of a population under the `om1` and `om2` operating models, and managed using the `mp_f40` and `mp_f50` management procedures. All simulations will use the same set of default model options, estimation method diagnostics will not be returned, and MSE outputs will be saved to disk:
